...
function renderList(titulo, lista) {
  const output = document.getElementById("output");
  
  let hmtl = "<h3>"+titulo+"</h3><ul>";
...
function filtrarStockbajo() {
  ...
  const filtroM = menu.filter(m = m.stock <= 3)
  if (filtroM.length === 0) {
    renderMenu("Stock BAJO")
  }
...
document.getElementById("btnConfirmarV").addEventListener("click", () => {
  renderMenu();
});
